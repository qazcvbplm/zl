...
import com.dingdian.order.ui.widget.bt.BluetoothActivity;
...
public class SearchBluetoothActivity extends BluetoothActivity implements AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState ) {
        ...
        sp = this.getSharedPreferences("gprs", Context.MODE_PRIVATE);
        lv_searchblt = (ListView) findViewById(R.id.lv_searchblt);
        gprs=(Switch) findViewById(R.id.gprs);
        tv_title = (TextView) findViewById(R.id.tv_title);
        tv_summary = (TextView) findViewById(R.id.tv_summary);
        ip=(EditText) findViewById(R.id.ip);
        ipB=(Button)findViewById(R.id.ipB);
        //初始化蓝牙适配器
        ...
        gprs.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if(b){
                    ...
                    editor.putBoolean("gprs",false);
                }
                editor.commit();
            }
        });
        gprs.setChecked(sp.getBoolean("gprs",false));
      ip.setText(sp.getString("ip","input ip"));
}
    @Override
    ...
    public void btFoundDevice(Intent intent) {
        ...
        if (null != bluetoothAdapter && device != null) {
            if (device.getName() == null) return;//过滤未知设备
//            if (device.getBluetoothClass().getMajorDeviceClass() != 1536) return;//过滤未知设备
